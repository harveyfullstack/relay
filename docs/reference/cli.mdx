...
| `agent-relay claude` | Start daemon with Claude coordinator |
| `agent-relay codex` | Start daemon with Codex coordinator |
...
Socket: /tmp/relay/abc123/agent-relay.sock
Relay tmux sessions:
- relay-Lead (agent: Lead, cwd: /Users/you/myproject)
...
Start the daemon with a Claude-based coordinator agent.
...
This command:
1. Checks if daemon is running, starts if needed
2. Installs the agent-relay snippet via prpm
3. Starts a coordinator agent using Claude

---

### agent-relay codex

Start the daemon with a Codex-based coordinator agent.

```bash
agent-relay codex
```

Same as `agent-relay claude` but uses OpenAI Codex CLI.
...
| `agent-relay spawn` | Spawn agent via API |
| `agent-relay release` | Release agent via API |
