...
**Option B: Multi-stage Docker build (RECOMMENDED)**
```dockerfile
...
# Final stage - copy binary only (no Rust toolchain)
FROM node:20-slim AS final
COPY --from=rust-builder /app/target/release/relay-pty /usr/local/bin/
# ... rest of workspace setup
```

**Option C: Cross-compile with musl (faster builds, requires local setup)**
```bash
# Build locally with static linking
cd relay-pty
cargo build --release --target x86_64-unknown-linux-musl
# Copy to deploy/workspace/relay-pty
```
```dockerfile
# In Dockerfile
...
**Decision: Option B (Multi-stage build)**
- Clean final image (no 500MB+ Rust toolchain)
- Source and binary always in sync
- Layer caching for fast rebuilds after first build
- Self-contained, no external dependencies or CI setup
...
2. **agent-relay-505** - 🚨 WORKSPACE_ID env propagation (P1 - required for namespacing)
