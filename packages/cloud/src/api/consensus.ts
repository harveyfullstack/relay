...
// Temporary shim until daemon consensus is extracted (Phase 6)
import type { Proposal } from '../shims/consensus.js';
...
consensusRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const proposalsMap = getProposalsForWorkspace(workspaceId);
      const proposals = Array.from(proposalsMap.values()).filter((p: Proposal) => {
        if (p.status !== 'pending') return false;
        if (!p.participants.includes(agentName)) return false;
        // Check if agent hasn't voted yet
        return !p.votes.some((v: { agent: string }) => v.agent === agentName);
      });
